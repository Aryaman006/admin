...
export default function ItemsPage() {
  const items = useSelector((state) => state.items.items); // get items from store
    ...
    <div className="p-6 md:p-10 bg-gray-50 min-h-screen font-sans">
      ...
      <div className="hidden md:block overflow-x-auto rounded-xl border border-gray-200 shadow bg-white">
        <table className="min-w-full border-collapse text-sm text-gray-700">
          ...
          <tbody>
            {items.map((i) => (
              <tr key={i.id} className="border-b last:border-b-0 hover:bg-gray-50 transition-colors duration-150">
